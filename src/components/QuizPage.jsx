...
function QuizPage() {
  ...
  const examStartTime = state?.examStartTime
    ...
    const checkDisqualification = async () => {
      try {
        const res = await apiFetch(
          `/my-rank?regNo=${encodeURIComponent(student.regNo)}`
        );
        ...
        console.debug(
          "Disqualification check skipped:",
          err.message
        );
      }
    };
  ...
  useEffect(() => {
    ...
    const checkStatus = async () => {
      try {
        const res = await apiFetch("/quiz-status");
        ...
        if (data.isQuizOpen) {
          ...
          if (data.startTime) {
            const diff = Math.ceil(
              (new Date(data.startTime) - new Date()) / 1000
            );
  ...
  const submitQuiz = useCallback(
    async (auto = false) => {
      ...
      try {
        ...
        questions.forEach(
          (q, shuffledIndex) => {
            originalAnswers[q.originalIndex] =
              answers[shuffledIndex];
          }
        );
        ...
        if (data.disqualified) {
          ...
          setDisqualifiedMessage(
            data.message ||
              "Time's up! You have been disqualified."
          );
